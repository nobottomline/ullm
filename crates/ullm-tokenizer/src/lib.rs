//! Tokenizers for uLLM.
//!
//! Phase 0 implements the SentencePiece (SPM) tokenizer used by the Llama
//! family, built from a GGUF file's `tokenizer.ggml.*` metadata: a score-based
//! greedy merge for encoding, with byte-fallback-aware decoding. Byte-level BPE
//! (the GPT-2 / Qwen style) will be added alongside the models that need it.
...
impl Tokenizer {
    ...
    /// Decode token ids back into text. Control/unknown tokens are dropped and
    /// the leading space introduced by the space prefix is removed.
