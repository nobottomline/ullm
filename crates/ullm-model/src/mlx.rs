...
use ullm_core::dequant::bf16_to_f32;
...
use crate::{LayerWeights, LlamaModel, MlxQuant, QWeight};
...
/// Wrap one MLX 4-bit linear (`<name>.weight/scales/biases`) into a resident
/// 4-bit [`QWeight`] — the packed u32 bytes are kept as-is and dequantized per
/// row during the matmul (4x less memory and bandwidth than expanding to BF16).
fn mlx_qweight(st: &SafeTensorsModel, name: &str, group_size: usize) -> Result<QWeight> {
    ...
    let data = st
        .tensor_data(&format!("{name}.weight"))
        .ok_or_else(|| Error::Format(format!("no data for '{name}.weight'")))?
        .to_vec();
...
/// Slice a stacked MLX expert tensor `[n_experts, out, in/8]` into one resident
/// 4-bit [`QWeight`] per expert.
fn mlx_experts(
    st: &SafeTensorsModel,
    name: &str,
    n_experts: usize,
    ...
    let scales = read_bf16(st, &format!("{name}.scales"))?;
    let biases = read_bf16(st, &format!("{name}.biases"))?;
    let wpe = out * (in_dim / 8) * 4; // weight bytes per expert
    let spe = out * (in_dim / group_size); // scales/biases per expert
    let mut experts = Vec::with_capacity(n_experts);
    for e in 0..n_experts {
        experts.push(QWeight {
            data: bytes[e * wpe..(e + 1) * wpe].to_vec(),
            dtype: DType::U32,
            out,
            cols: in_dim,
            mlx: Some(MlxQuant {
                scales: scales[e * spe..(e + 1) * spe].to_vec(),
                biases: biases[e * spe..(e + 1) * spe].to_vec(),
                group_size,
            }),
        });
    }
