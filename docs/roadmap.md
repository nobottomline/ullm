...
- ◐ Metal backend — validated GEMV + unified-memory buffers (`ullm metal-check`); attention/dequant kernels + forward integration next
- ☐ KV cache + sampler
- ◐ OpenAI-compatible server — `/v1/chat/completions` + `/v1/models` (`ullm serve`); SSE streaming next
